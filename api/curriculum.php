...
$subStmt = db()->prepare('SELECT * FROM subjects WHERE program_code = ? ORDER BY year_level, semester, subject_id');
$subStmt->execute([$programCode]);
$subjects = $subStmt->fetchAll();

$prereqStmt = db()->prepare('SELECT p.sub_code FROM subject_prerequisites sp JOIN subjects p ON p.subject_id = sp.prereq_subject_id WHERE sp.subject_id = ? ORDER BY sp.id');
